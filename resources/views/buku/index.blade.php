...
<body>
    ...
    <div class="container">
        <h3>Daftar buku</h3>
        <a href="{{ route('buku.create') }}" class="create">Tambah data</a>
        <form>
            <table class="table">
                  <tr>
                      <td>No</td>
                      <td width="300px">Judul buku</td>
                      <td>Penerbit</td>
                      <td>Pengarang</td>
                  ...
                  <tr>
                      ...
                      <td> {{ $b->penerbit}} </td>
                      <td> {{ $b->pengarang}} </td>
                      <td><a href="{{ route('buku.edit', $b->id) }}" class="btn btn-primary">EDIT</a></td> 
                      <td>
                          <form onsubmit="return confirm('Yakin Hapus {{ $b->judul_buku }}??')" action="{{ route('buku.destroy', $b->id) }} " method="POST">
                          @csrf
                          @method('DELETE')
